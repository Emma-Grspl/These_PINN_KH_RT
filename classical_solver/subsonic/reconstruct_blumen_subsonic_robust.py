...
from classical_solver.subsonic.robust_subsonic_shooting import sample_robust_subsonic_growth_map


DATA_DIR = ROOT_DIR / "KH_RT_Blumen" / "subsonic"
OUTPUT_DIR = ROOT_DIR / "assets" / "blumen_shooting_robust"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reconstruction Blumen 1970 avec solveur subsonique robuste.")
...
def plot_map(df: pd.DataFrame, curves: list[dict], output_path: Path) -> None:
    ...
    ax.set_title("Blumen 1970 : reconstruction subsonique robuste")
...
def main() -> None:
    ...
    print("Echantillonnage subsonique robuste...")
    df = sample_robust_subsonic_growth_map(alphas, machs, force_cross_check=args.force_cross_check)

    csv_path = OUTPUT_DIR / "subsonic_robust_growth_map.csv"
    fig_path = OUTPUT_DIR / "subsonic_robust_vs_blumen.png"
    error_level_path = OUTPUT_DIR / "subsonic_robust_error_by_level.csv"
    error_point_path = OUTPUT_DIR / "subsonic_robust_error_by_point.csv"
    error_summary_path = OUTPUT_DIR / "subsonic_robust_error_summary.json"
