...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run stabilized first-order real subsonic KH PINN with sparse ci supervision."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("model_saved/kh_subsonic_fixed_mach_M05_first_order_real_stabilized"),
    )
...
def build_config(args: argparse.Namespace) -> KHSubsonicTrainingConfig:
    return KHSubsonicTrainingConfig(
        ...
        mode_center_fraction=0.7,
        mode_center_half_width=0.35,
        w_pde=1.0,
        w_bc=20.0,
        w_norm=25.0,
        w_integral_norm=20.0,
        w_phase=0.0,
        w_peak_slope=0.0,
        w_peak_curvature=0.0,
        w_loc_center=0.1,
        w_loc_spread=0.02,
        w_first_order_v_energy=0.02,
        w_first_order_amp_cap=10.0,
        ...
        classic_xi_max=0.99,
        mode_representation="first_order_real",
        separate_branch_optimizers=True,
        detach_ci_in_mode_branch=True,
        ci_branch_lr=1e-3,
        mode_branch_lr=args.learning_rate,
...
def summarize_history(history: pd.DataFrame) -> dict[str, float]:
    ...
    if audited.empty:
        return {
            ...
            "best_audit_env": np.nan,
            "best_audit_phase": np.nan,
            "last_epoch": -1,
            "last_ci_mae": np.nan,
            "last_p_rel": np.nan,
            "last_env": np.nan,
    ...
    best_idx = int(pd.to_numeric(audited["audit_checkpoint_metric"], errors="coerce").idxmin())
    ...
    return {
        ...
        "best_audit_env": float(best_row["audit_env_rel_mean"]),
        "best_audit_phase": float(best_row["audit_phase_rel_mean"]),
...
def main() -> None:
    ...
    summary_path = args.output_dir / "first_order_real_stabilized_summary.csv"
    pd.DataFrame([summary]).to_csv(summary_path, index=False)

    print(
        "best_epoch={best_epoch} best_ci_mae={best_audit_ci_mae:.3e} best_p_rel={best_audit_p_rel:.3e} "
        "best_env={best_audit_env:.3e} best_phase={best_audit_phase:.3e}".format(**summary)
    )
    print(
        "last_epoch={last_epoch} last_ci_mae={last_ci_mae:.3e} last_p_rel={last_p_rel:.3e} "
        "last_env={last_env:.3e} last_phase={last_phase:.3e}".format(**summary)
    )
