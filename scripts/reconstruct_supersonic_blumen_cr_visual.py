...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Reconstruction visuelle propre des isolignes supersoniques de Blumen en c_r."
    )
    ...
    parser.add_argument("--cr-max", type=float, default=0.55)
    parser.add_argument("--tracking-weight", type=float, default=2e-2)
    parser.add_argument("--output-stem", type=str, default="supersonic_blumen_cr_visual")
    return parser


def load_cr_curves() -> list[dict]:
    curves = load_digitized_curves(DATA_DIR)
    return [
        curve
        for curve in curves
        if curve["family"] in {"cr_level", "ci_special", "cr_special"} and curve["level"] is not None
    ]
...
def contour_levels(curves: list[dict]) -> list[float]:
    return sorted({float(curve["level"]) for curve in curves if curve["family"] == "cr_level"})
...
def plot_blumen_reference(curves: list[dict], output_path: Path) -> None:
    ...
    main_levels = [curve for curve in curves if curve["family"] == "cr_level"]
    targets = [
        (0.10, 1.22, "0·10"),
        (0.20, 1.33, "0·20"),
        (0.30, 1.50, "0·30"),
    ...
    ax.set_title(r"Blumen 1975: isolignes supersoniques de $c_r$")
...
def plot_shooting_overlay(df: pd.DataFrame, curves: list[dict], output_path: Path) -> None:
    pivot = df.pivot(index="alpha", columns="Mach", values="cr").sort_index().sort_index(axis=1)
    machs = pivot.columns.to_numpy(dtype=float)
    alphas = pivot.index.to_numpy(dtype=float)
    cr_values = pivot.to_numpy(dtype=float)
    ...
    ax.set_title(r"Reconstruction visuelle: isolignes de $c_r$ du shooting vs Blumen")
    ax.grid(True, linestyle=":", alpha=0.25)

    legend_lines = [
        plt.Line2D([], [], color="black", linewidth=1.4, label=r"Blumen $c_r$ (principal)"),
        ...
        plt.Line2D([], [], color=plt.get_cmap("magma")(0.7), linewidth=2.0, label=r"Shooting $c_r$"),
    ]
...
def main() -> None:
    ...
    print("Reconstruction visuelle Blumen supersonique en c_r")
    ...
    print(f"c_r levels: {' '.join(f'{level:.2f}' for level in contour_levels(curves))}")
