...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Mode repair high-alpha cible pour le PINN subsonique a alpha=0.85."
    )
    parser.add_argument("--mach", type=float, default=0.5)
    parser.add_argument("--alpha-target", type=float, default=0.85)
    parser.add_argument("--alpha-min", type=float, default=0.845)
    ...
    parser.add_argument(
        "--warmstart-run-dir",
        type=Path,
        default=Path("model_saved/kh_subsonic_fixed_mach_M05_highalpha_stepwise/stage_02_alpha_0p840"),
    )
...
def main() -> None:
    ...
    print(
        "Warm-start mode-repair metrics "
    ...
    summary_path = Path(args.output_dir) / "mode_repair_summary.csv"
    pd.DataFrame([summary]).to_csv(summary_path, index=False)

    print(
        f"regime=high_alpha_mode_repair alpha_target={float(args.alpha_target):.3f} "
