...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Supervision directe du mode classique a high-alpha pour le PINN subsonique."
    )
    ...
    parser.add_argument("--mode-supervision-weight", type=float, default=20.0)
    parser.add_argument("--mode-supervision-n-xi", type=int, default=257)
    parser.add_argument("--mode-supervision-y-max", type=float, default=8.0)
    parser.add_argument(
        "--warmstart-run-dir",
        type=Path,
...
def build_config(args: argparse.Namespace, initial_model_path: Path) -> KHSubsonicTrainingConfig:
    ...
    return KHSubsonicTrainingConfig(
        ...
        classic_mode_supervision_n_xi=int(args.mode_supervision_n_xi),
        classic_mode_supervision_xi_min=-0.98,
        classic_mode_supervision_xi_max=0.98,
        classic_mode_supervision_y_max=float(args.mode_supervision_y_max),
        w_classic_mode_supervision=float(args.mode_supervision_weight),
...
def main() -> None:
    ...
    print(
        "Warm-start classic-mode supervision metrics "
    ...
    summary = {
        "regime": "high_alpha_classic_mode_supervision",
        "mach": float(args.mach),
        "alpha_min": float(args.alpha_min),
        "alpha_max": float(args.alpha_max),
        "alpha_target": float(args.alpha_target),
        ...
        "freeze_ci": True,
        "mode_supervision_weight": float(args.mode_supervision_weight),
        "mode_supervision_n_xi": int(args.mode_supervision_n_xi),
        "mode_supervision_y_max": float(args.mode_supervision_y_max),
    ...
    summary_path = Path(args.output_dir) / "classic_mode_supervision_summary.csv"
    pd.DataFrame([summary]).to_csv(summary_path, index=False)

    print(
        f"regime=high_alpha_classic_mode_supervision alpha_target={float(args.alpha_target):.3f} "
