...
python3 scripts/train_kh_subsonic_pinn.py \
  ...
  --w-riccati-anchor 1.0 \
  --riccati-anchor-supervision \
  --riccati-anchor-n-xi 49 \
  --riccati-anchor-every 20 \
  --riccati-anchor-alphas 0.20 0.65 \
  --w-q-supervision 1.0 \
  --q-supervision-n-xi 97 \
  --q-supervision-every 20 \
  --q-supervision-alpha-count 6 \
  --mode-low-alpha-threshold 0.25 \
  --mode-low-alpha-weight 2.5 \
  --mode-low-alpha-audit-fraction 0.67 \
  ...
  --device "${DEVICE:-cpu}" \
  --output-dir "${OUTPUT_DIR:-model_saved/kh_subsonic_fixed_mach_M05_riccati_experts_q_lowalpha}"
