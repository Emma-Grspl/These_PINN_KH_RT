    ...
    *,
    ...
    lines = [
        ...
        (
            ...
            f"center_pde={cfg.w_center_pde} sym={cfg.w_sym} "
            f"center_deriv={cfg.w_center_deriv} ci_anchor={cfg.w_ci_anchor}"
        ),
        f"ymax={cfg.ymax}",
        ...
        f"best_loss_sym={float(best_history_row.get('loss_sym', float('nan'))):.8e}",
        f"best_loss_center_deriv={float(best_history_row.get('loss_center_deriv', float('nan'))):.8e}",
        ...
        "- optional physical parity regularization: Re p even, Im p odd",
        "- optional center derivative parity: Re p_y(0)=0",
...
def train_kh_subsonic_2d_stage1quater_pressure(
    ...
    for epoch in range(1, int(cfg.epochs) + 1):
        ...
        n_sym = max(int(cfg.n_sym), 0)
        if n_sym > 0:
            sym_ymax = min(float(cfg.sym_ymax), float(cfg.ymax))
            n_sym_uniform = n_sym // 2
            n_sym_center = n_sym - n_sym_uniform
            y_sym_uniform = torch.empty(n_sym_uniform, 1, device=device).uniform_(0.0, sym_ymax)
            y_sym_center = torch.abs(
                torch.randn(n_sym_center, 1, device=device) * max(0.25 * sym_ymax, 1.0e-6)
            ).clamp(max=sym_ymax)
            y_sym_base = torch.cat([y_sym_uniform, y_sym_center], dim=0)
            y_sym, alpha_sym, mach_sym = _repeat_spatial_over_param_pairs(
                y_sym_base, alpha_pairs, mach_pairs
            )
        else:
            y_sym_base = torch.empty(0, 1, device=device)
            y_sym = torch.empty(0, 1, device=device)
            alpha_sym = torch.empty(0, 1, device=device)
            mach_sym = torch.empty(0, 1, device=device)
        ...
        loss_sym = zero
        if y_sym.numel() > 0:
            ci_sym = ci_override_pairs.repeat_interleave(int(y_sym_base.shape[0]), dim=0)
            loss_sym = pressure_first_symmetry_loss(
                model,
                y_sym,
                alpha_sym,
                mach_sym,
        ...
        loss_center_deriv = pressure_first_center_derivative_loss(
            model,
            alpha_pairs,
            mach_pairs,
            ci_override=ci_override_pairs,
        ...
        loss_total = (
            ...
            + float(cfg.w_sym) * loss_sym
            + float(cfg.w_center_deriv) * loss_center_deriv
        ...
        row: dict[str, float | int] = {
            ...
            "loss_center_deriv": float(loss_center_deriv.detach().cpu()),
        ...
        if epoch == 1 or epoch % int(cfg.audit_every) == 0 or epoch == int(cfg.epochs):
            print(
                ...
                f"| center={row['loss_center_pde']:.3e} | sym={row['loss_sym']:.3e} "
                f"| center_deriv={row['loss_center_deriv']:.3e} "
                f"| ci_max_abs={row['ci_anchor_max_abs']:.3e} "
